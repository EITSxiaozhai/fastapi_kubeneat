...
### 9.4 任务中心能看到旧任务，但 `Submit type` 显示为 `Unknown`

这是兼容行为。早期写入的任务记录没有 `submission_type` 字段，新版本只会为新任务写入该字段。

## 10. 维护建议

当前版本的任务注册表使用 `jsonl` 文件保存元信息，适合单机、轻量部署和开发环境。

如果后续需要支持以下场景，建议单独引入数据库表而不是继续扩展文件注册表：
...
在当前规模下，`FastAPI + Celery + Redis + jsonl 注册表` 已经足够满足本地开发、容器部署和基础任务看板需求。
